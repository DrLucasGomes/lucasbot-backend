...
def _request_e2e_autorizado():
    request = FakeRequest()
    request.headers = {"Authorization": "Bearer worker-secret"}
    return request


def _venda_e2e_valida():
    return {
        "id": "order-e2e",
        "status": "waiting_payment",
        "payment_method": "pix",
...
def test_e2e_direto_exige_auth_e_nao_confirma_venda(monkeypatch):
    chamadas = []
    monkeypatch.setenv("PIX_RECOVERY_WORKER_TOKEN", "worker-secret")
    monkeypatch.setattr(
        pix_recovery,
    ...
    with pytest.raises(Exception) as erro:
        asyncio.run(
            pix_recovery.e2e_kit_first_name_endpoint("order-e2e", FakeRequest())
        )
...
def test_e2e_direto_venda_invalida_retorna_resultado_sanitizado(monkeypatch):
    monkeypatch.setenv("PIX_RECOVERY_WORKER_TOKEN", "worker-secret")
    monkeypatch.setattr(pix_recovery, "confirmar_venda_kiwify", lambda *a, **k: {})
    monkeypatch.setattr(
        pix_recovery,
        "_alterar_tag_kit_detalhado",
        lambda *a, **k: pytest.fail("Kit nao deveria ser chamado"),
    )

    resultado = asyncio.run(
        pix_recovery.e2e_kit_first_name_endpoint(
            "order-e2e", _request_e2e_autorizado()
        )
    )
...
def test_e2e_direto_confirma_venda_com_contrato_oficial(monkeypatch):
    confirmacoes = []
    monkeypatch.setenv("PIX_RECOVERY_WORKER_TOKEN", "worker-secret")

    def confirmar(order_id, statuses_aceitos, payment_method_esperado=None):
        confirmacoes.append((order_id, statuses_aceitos, payment_method_esperado))
        return _venda_e2e_valida()

    monkeypatch.setattr(pix_recovery, "confirmar_venda_kiwify", confirmar)
    monkeypatch.setattr(
        pix_recovery, "_alterar_tag_kit_detalhado", lambda *a: (True, True, True)
    )
    ...
    assert confirmacoes == [
        ("order-e2e", pix_recovery.KIWIFY_PENDING_STATUSES, "pix")
    ]


def test_e2e_direto_tag_e_put_sucesso_sem_email_no_put(monkeypatch):
    puts = []
    monkeypatch.setenv("PIX_RECOVERY_WORKER_TOKEN", "worker-secret")
    monkeypatch.setenv("TAG_PIX_ID", "tag-pix")
    monkeypatch.setenv("CONVERTKIT_API_SECRET", "secret-value")
    monkeypatch.setattr(
        pix_recovery, "confirmar_venda_kiwify", lambda *a, **k: _venda_e2e_valida()
    )
    monkeypatch.setattr(
        pix_recovery.requests,
        "post",
    ...
    monkeypatch.setattr(
        pix_recovery.requests,
        "put",
        lambda url, **kwargs: puts.append(kwargs["json"]) or FakeResponse(200),
    )
    ...
    assert resultado == {
        ...
        "first_name_sync_success": True,
    }
    assert puts == [{"api_secret": "secret-value", "first_name": "Maria"}]
    assert "email" not in puts[0]


@pytest.mark.parametrize("put_result", [FakeResponse(500), TimeoutError("timeout")])
def test_e2e_direto_falha_put_nao_quebra_tag(monkeypatch, put_result):
    monkeypatch.setenv("PIX_RECOVERY_WORKER_TOKEN", "worker-secret")
    monkeypatch.setenv("TAG_PIX_ID", "tag-pix")
    monkeypatch.setenv("CONVERTKIT_API_SECRET", "secret-value")
    monkeypatch.setattr(
...
def test_e2e_direto_nao_toca_jobs_ledger_ou_handler(monkeypatch):
    monkeypatch.setenv("PIX_RECOVERY_WORKER_TOKEN", "worker-secret")
    monkeypatch.setattr(
        pix_recovery, "confirmar_venda_kiwify", lambda *a, **k: _venda_e2e_valida()
    )
    ...
    def proibido(*args, **kwargs):
        pytest.fail("endpoint E2E tocou estado durable ou handler legado")
    ...
    assert resultado["tag_success"] is True
