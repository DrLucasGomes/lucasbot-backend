...
def adicionar_lead_convertkit(email: str, first_name: str = ""):
    ...
    try:
        ...
        subscribe_success = r.status_code in (200, 201, 204)
        print(
            f"stage=kit_subscribe_completed success={subscribe_success}",
            flush=True,
        )
        ...
        if r.status_code in (200, 201, 204) and first_name:
            ...
            print(
                f"stage=subscriber_id_valid value={subscriber_id is not None}",
                flush=True,
            )
            if subscriber_id is not None:
                atualizar_first_name_kit(subscriber_id, first_name)
        else:
            print("stage=subscriber_id_valid value=False", flush=True)
    except Exception as e:
        print("stage=kit_subscribe_completed success=False", flush=True)
